<x-admin-layout title="Edit Monitored Website: {{ $site->name }}" breadcrumb="Update website configuration and monitoring interval">
    ...
    <div style="max-width:820px;">
        ...
        <form action="{{ route('admin.sites.update', $site->id) }}" method="POST">
            ...
            <div class="card" style="margin-bottom:20px;">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" value="{{ old('name', $site->name) }}" required class="form-input" style="width:100%;">
                    </div>

                    <div class="form-group" style="margin-top:16px;">
                        <label class="form-label" style="font-weight:600; margin-bottom:6px; display:block;">Target URL *</label>
                        <input type="url" name="url" value="{{ old('url', $site->url) }}" required class="form-input" style="width:100%;">
                    </div>
                    ...
                    <div class="form-group" style="margin-top:16px;">
                        ...
                        <select name="check_interval" required class="form-input" style="width:100%;">
                            <option value="1" {{ old('check_interval', $site->check_interval) == 1 ? 'selected' : '' }}>Every 1 Minute</option>
                            <option value="5" {{ old('check_interval', $site->check_interval) == 5 ? 'selected' : '' }}>Every 5 Minutes (Standard)</option>
                            <option value="15" {{ old('check_interval', $site->check_interval) == 15 ? 'selected' : '' }}>Every 15 Minutes</option>
                            <option value="30" {{ old('check_interval', $site->check_interval) == 30 ? 'selected' : '' }}>Every 30 Minutes</option>
                            <option value="60" {{ old('check_interval', $site->check_interval) == 60 ? 'selected' : '' }}>Every 60 Minutes (1 Hour)</option>
                        </select>
                    </div>

                    <div class="form-group" style="margin-top:16px;">
                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:13px; font-weight:600; color:var(--text-primary);">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $site->is_active) ? 'checked' : '' }} style="width:16px; height:16px; accent-color:var(--accent);">
                            Enable Active Automated Checks
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div style="display:flex; justify-content:flex-end; gap:12px; margin-top:24px;">
                ...
                <button type="submit" class="topbar-btn topbar-btn-primary">Update Configuration</button>
            </div>
        </form>
    </div>
</x-admin-layout>
